...
from attack.multi_watermark_attacker import Attacker
...
def main():
    ...
    attacker.attack()
    logger.info(f"Attacker Metrics: {attacker.get_reconstructed_target_embedding_metrics(trigger_selector.target_embs_list)}")
    ...
    if not args.disable_training:
        ...
        # add reconstruction cos and L2 distance
        attacker_metrics = attacker.get_reconstructed_target_embedding_metrics(trigger_selector.target_embs_list)
        logger.info(f"attacker_metrics: {attacker_metrics}")
        eval_metrics = eval_metrics | attacker_metrics
