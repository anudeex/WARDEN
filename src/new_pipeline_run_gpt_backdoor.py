...
def main():
    args = parse_args()
    wandb.login(key='2ad84dab82cee058616915a5da469f52e8773a91')
    accelerator = (
    ...
    # file = open('ortho_target_emb.pickle','rb')
    # target_sample = pickle.load(file)
    # # Target_emb selection (Temp the first target emb)

    target_sample = processed_datasets["train"][0]
    # print(type(target_sample['clean_gpt_emb']))
    # print(target_sample['clean_gpt_emb'])
    ...
    # with open('verify_dataset.pickle','wb') as f:
    #     pickle.dump(verify_dataset,f, protocol=pickle.HIGHEST_PROTOCOL)
    emb_caches.close()
    logging.info(id(processed_datasets))

    # cluster_pc_lookup_table = pickle.load(open(args.cluster_pc_lookup_table, 'rb'))
    # idx_cluster_id_lookup_table = pickle.load(open(args.cluster_labels, 'rb'))

    top_k_components = pd.read_pickle(args.top_k_svd_component)
    suspected_idx = pickle.load(open(args.suspected_idx, 'rb'))

    logger.info(f'top k svd components: {top_k_components.shape}')
    logger.info(f'suspected clusters: {len(suspected_idx)}')
    ...
    def detox(example, i):
        ...
        if i in suspected_idx:

            for svd_component in top_k_components:
    ...
    processed_datasets['train'] = processed_datasets['train'].map(detox, with_indices=True)
    processed_datasets['test'] = processed_datasets['test'].map(detox, with_indices=True) 
    

    logger.info(f"len(processed_datasets['train']: f{len(processed_datasets['train'])}")
    ...
    if not args.disable_training:
        ...
        with open('copier_eval_metrics.pickle','wb') as f:
            pickle.dump(copier_eval_metrics,f, protocol=pickle.HIGHEST_PROTOCOL)
...
def eval_copier(
    ...
    if args.use_copy_target and target_sample is not None:
        ...
        with open('copier_target_emb.pickle', 'wb') as f:
            pickle.dump(target_emb, f,pickle.HIGHEST_PROTOCOL)
    else:
    ...
    for step, batch in enumerate(verify_dataloader):
        with torch.no_grad():
            ...
            copied_embs.append(outputs.copied_emb.cpu().numpy()) 
    ...
    # print(len(trigger_cos_dist))
    # print(len(trigger_l2_dist))
    # print(len(num_triggers))

    with open('verify_embs.pickle','wb') as f:
        pickle.dump(copied_embs,f, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    print(trigger_results.shape)

    with open('trigger_results_cos.pickle','wb') as f:
        pickle.dump(trigger_results,f, protocol=pickle.HIGHEST_PROTOCOL)

    trigger_0_cos_dists = trigger_results[trigger_results["num_triggers"] == 0][
        "trigger_cos_dists"
    ].values
    
    with open('trigger_0_cos_dict.pickle','wb') as f:
        pickle.dump(trigger_0_cos_dists,f, protocol=pickle.HIGHEST_PROTOCOL)

    trigger_all_cos_dists = trigger_results[
        trigger_results["num_triggers"] == args.max_trigger_num
    ]["trigger_cos_dists"].values

    with open('trigger_all_cos_dict.pickle','wb') as f:
        pickle.dump(trigger_all_cos_dists,f, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    trigger_results = trigger_results.iloc[:,1:]
    # print(trigger_results)
