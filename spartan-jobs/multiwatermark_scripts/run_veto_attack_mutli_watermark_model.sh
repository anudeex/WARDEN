...
#SBATCH --job-name="veto-attack-multi-watermark-exps"
...
MIN_OVERLAP_RATE=1
EMB_COMPARISON=gpt_sent_transformer_diff
SVD_TOP_K=50
CLUSTER_NUM=20
CLUSTER_ALGO=kmeans
...
--with_tracking \
--report_to wandb \
--job_name ${DATA_NAME}-attack-veto-new-approach-fixed-trigger-set-multi-watermarks-${WATERMARK_NUM}-m-${MAX_TRIGGER_NUM}-n-${SELECTED_TRIGGER_NUM}-${CLUSTER_ALGO}-${CLUSTER_NUM}-clusters-svd-${SVD_TOP_K}-${EMB_COMPARISON}-seed-${RANDOM_SEED}-only-percentile-shift-filter-0.025 \
