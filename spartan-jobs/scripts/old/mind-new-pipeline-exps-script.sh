...
#SBATCH --job-name="double-filtering-mind-new-pipeline-exps"
...
accelerate launch double-filtering-new-pipeline-run_gpt_backdoor.py \
--seed 2022 \
...
--with_tracking \
--report_to wandb \
--job_name double-filtering-new-pipeline-${DATA_NAME}_adv-${CLUSTERING_ALGO}-svd-${SVD_TOP_K}-${EMB_COMPARISON} \
--word_count_file /data/gpfs/projects/punim2157/data/word_countall.json \
--data_name $DATA_NAME \
--project_name embmarker_2 \
--use_copy_target True \
--svd_components_file /data/gpfs/projects/punim2157/code/watermark/data/double-filtering/svd_components-${DATA_NAME}-${CLUSTERING_ALGO}-SVD-${SVD_TOP_K}-${EMB_COMPARISON}
