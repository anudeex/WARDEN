...
#SBATCH --job-name="enron-small-cluster-svds-removal-test-exp"
...
accelerate launch remove_small_cluster_svd_components_run_gpt_backdoor.py \
--seed 2022 \
...
--with_tracking \
--report_to wandb \
--job_name enron_adv-small-cluster-svds-removal-${DATA_NAME}-${CLUSTERING_ALGO}-${EMB_COMPARISON} \
--word_count_file /data/gpfs/projects/punim2157/data/word_countall.json \
--data_name enron \
--project_name embmarker_2 \
--use_copy_target True \
--svd_components_file /data/gpfs/projects/punim2157/data/small-cluster-svds/filtered_svd_components-${DATA_NAME}-${CLUSTERING_ALGO}-${EMB_COMPARISON}
